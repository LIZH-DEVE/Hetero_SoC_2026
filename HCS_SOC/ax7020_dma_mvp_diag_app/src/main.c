...
static void print_stage(const char *stage)
{
    xil_printf("stage=%s\r\n", stage);
}
...
int main(void)
{
    ...
    xil_printf("DMA diag image\r\n");
    print_stage("main_enter");
    ...
    if (rc != 0) {
        xil_printf("DMA diag FAIL at normal transaction\r\n");
        return 1;
    }

    rc = run_error_transaction(&ctx);
    if (rc != 0) {
        xil_printf("DMA diag FAIL at error transaction\r\n");
        return 2;
    }

    xil_printf("DMA diag PASS\r\n");
    return 0;
}
