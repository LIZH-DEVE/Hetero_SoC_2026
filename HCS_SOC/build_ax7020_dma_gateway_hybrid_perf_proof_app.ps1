...
$appSrcDir = Join-Path $workspace "ax7020_dma_gateway_hybrid_perf_proof_app\src"
$buildDir = Join-Path $workspace "ax7020_dma_gateway_hybrid_perf_proof_app\build"
...
function Resolve-HybridApiBspRoot {
    ...
    throw "No hybrid proof platform API BSP root found under $WorkspaceRoot."
}
...
function Resolve-HybridToolchainRoot {
    ...
    throw "No hybrid proof toolchain BSP root with libxil.a found under $WorkspaceRoot"
}
...
function Resolve-SpecsFile {
    ...
    throw "Xilinx.spec could not be resolved from hybrid proof platform root: $StartDir"
}
...
$toolchainRoot = Resolve-HybridToolchainRoot -WorkspaceRoot $workspaceRoot -PlatformName $defaultPlatformName
...
$elfPath = Join-Path $buildDir "ax7020_dma_gateway_hybrid_perf_proof_app.elf"
$mapPath = Join-Path $buildDir "ax7020_dma_gateway_hybrid_perf_proof_app.map"
$libs = @()
...
if ($LASTEXITCODE -ne 0) {
    throw "Compile failed for hybrid perf proof main.c"
}
...
& $gcc @linkArgs
if ($LASTEXITCODE -ne 0) {
    throw "Link failed for ax7020_dma_gateway_hybrid_perf_proof_app"
}
...
Write-Host "Built hybrid perf proof app:"
Write-Host $elfPath
