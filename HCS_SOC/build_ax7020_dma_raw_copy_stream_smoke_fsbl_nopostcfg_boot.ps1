...
# Dedicated stream-smoke FSBL no-post-config release contract:
# - boot image is fixed to [bootloader] fsbl.elf + stream_smoke_dma_wrapper.bit + ax7020_dma_raw_copy_stream_smoke_app.elf
# - variant intentionally bypasses ps7_post_config on the with-bit handoff path
...
if ([string]::IsNullOrWhiteSpace($WorkspaceRoot)) {
    $WorkspaceRoot = Join-Path $workspace "ax7020_dma_stream_smoke_fsbl_nopostcfg_xsct\workspace"
}
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $OutputDir = Join-Path $workspace "sd_boot\ax7020_dma_raw_copy_stream_smoke_fsbl_nopostcfg"
}
