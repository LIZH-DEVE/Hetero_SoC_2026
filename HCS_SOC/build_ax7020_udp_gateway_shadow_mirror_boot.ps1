...
function Resolve-ShadowFsblFallback {
    ...
    return Get-ChildItem -Path $Workspace -Recurse -File -Filter fsbl.elf -ErrorAction SilentlyContinue |
        Sort-Object LastWriteTime -Descending |
        Select-Object -First 1
}
...
function Resolve-ShadowBitstream {
    param(
        [string]$XsaExtractDir,
        [string]$Workspace
    )

    $bitstream = Get-ChildItem -Path $XsaExtractDir -File -Filter *.bit -ErrorAction SilentlyContinue |
        Sort-Object LastWriteTime -Descending |
        Select-Object -First 1
    if ($null -ne $bitstream) {
        return $bitstream
    }
    ...
    throw "No bitstream found inside shadow mirror XSA or manual impl fallback under $Workspace"
}
...
if (-not [string]::IsNullOrWhiteSpace($PlatformSwDir)) {
    if (-not (Test-Path (Join-Path $PlatformSwDir "include\xparameters.h"))) {
        throw "Explicit PlatformSwDir is not a valid BSP root: $PlatformSwDir"
    }
    $fsblElf = Resolve-ShadowFsblElf -WorkspaceRoot $WorkspaceRoot -PlatformName $PlatformName
    if ($null -eq $fsblElf) {
        $fsblElf = Resolve-ShadowFsblFallback -Workspace $workspace
    }
    if ($null -eq $fsblElf) {
        throw "Shadow mirror fsbl.elf not found under $WorkspaceRoot or $workspace"
    }
    Write-Host "Using explicit shadow mirror app BSP root:"
...
$bitstream = Resolve-ShadowBitstream -XsaExtractDir $xsaExtractDir -Workspace $workspace
