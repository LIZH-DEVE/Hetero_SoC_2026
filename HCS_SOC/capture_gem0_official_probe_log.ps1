...
if (-not $OutputPath) {
    ...
    $OutputPath = Join-Path $repoRoot ("board_gem0_official_probe_{0}_{1}.txt" -f $Baud, $stamp)
}
...
try {
    ...
    if ($ProgramBoard) {
        ...
        try {
            & (Join-Path $workspace "run_gem0_official_probe_app.ps1") @runnerArgs
        } catch {
            $runnerFailure = $_
        }
    }
...
if ($runnerFailure) {
    if ($preview -match "GEM0_OFFICIAL_PROBE_DONE") {
        Write-Warning "xsdb reported failure, but official GEM0 probe completed and UART evidence was captured; treating capture as usable."
        return
    }
    throw $runnerFailure
}
