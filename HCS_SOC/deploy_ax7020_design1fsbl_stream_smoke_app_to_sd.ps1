...
param(
    [string]$SdDrive = "E:",
    [string]$BackupDir = "D:\FPGAhanjia\Hetero_SoC_2026_3\sdcard_backup_20260327_design1fsbl_stream_smoke_app"
)
...
$releaseDir = Join-Path $workspace "sd_boot\ax7020_design1fsbl_stream_smoke_app"
...
Assert-PathType -Path $releaseDir -PathType Container -Label "design1fsbl-stream-smoke-app release directory"
foreach ($leafPath in @($sourceBoot, $sourceFsbl, $sourceBit, $sourceApp, $sourceBootRead, $sourceReadme)) {
    Assert-PathType -Path $leafPath -PathType Leaf -Label "design1fsbl-stream-smoke-app release artifact"
}
...
Assert-TextContains -Path $sourceReadme -RequiredText @("design1-FSBL", "DMA stream smoke image") -Label "readme.txt" | Out-Null
...
Write-Host "design1fsbl-stream-smoke-app release manifest:"
...
if (Test-Path -Path $targetBoot -PathType Leaf) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_before_design1fsbl_stream_smoke_app_$timestamp.bin"
...
Write-Host "Deployed design1fsbl-stream-smoke-app BOOT.BIN to SD:"
Write-Host $targetBoot
Write-Host "SHA256: $targetHash"
Write-Host "Current status:"
Write-Host "  - this is the temporary board-proven Phase 3 baseline"
Write-Host "Expected board-side behavior:"
Write-Host "  - DMA stream smoke image"
Write-Host "  - STREAM_STAGE EXACT_FIT PASS actual_len=1024"
Write-Host "  - STREAM_STAGE SHORT PASS actual_len=64"
Write-Host "  - STREAM_STAGE OVERFLOW PASS actual_len=64"
Write-Host "  - DMA stream smoke PASS"
