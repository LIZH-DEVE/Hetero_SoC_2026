...
param(
    [string]$SdDrive = "E:",
    [string]$BackupDir = "D:\FPGAhanjia\Hetero_SoC_2026_3\sdcard_backup_dma_gateway_hybrid_smoke"
)
...
$sourceBoot = Join-Path $workspace "sd_boot\ax7020_dma_gateway_hybrid_smoke\BOOT.BIN"
$sourceBif = Join-Path $workspace "sd_boot\ax7020_dma_gateway_hybrid_smoke\boot.bif"
$sourceReadme = Join-Path $workspace "sd_boot\ax7020_dma_gateway_hybrid_smoke\readme.txt"
$sdDriveRoot = Resolve-SdDriveRoot -ConfiguredDrive $SdDrive
$targetBoot = Join-Path $sdDriveRoot "BOOT.BIN"

Assert-PathType -Path $releaseDir -PathType Container -Label "DMA gateway hybrid smoke release directory"
foreach ($leafPath in @($sourceBoot, $sourceBif, $sourceReadme)) {
    Assert-PathType -Path $leafPath -PathType Leaf -Label "DMA gateway hybrid smoke release artifact"
}
...
Write-Host "DMA gateway hybrid smoke release manifest:"
...
if (Test-Path -Path $targetBoot -PathType Leaf) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_before_dma_gateway_hybrid_smoke_$timestamp.bin"
...
Write-Host "Deployed DMA gateway hybrid smoke BOOT.BIN to SD:"
