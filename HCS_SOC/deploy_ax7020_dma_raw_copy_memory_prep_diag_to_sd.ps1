...
param(
    [string]$SdDrive = "E:",
    [string]$BackupDir = "D:\FPGAhanjia\Hetero_SoC_2026_3\sdcard_backup_20260325_dma_raw_copy_memory_prep_diag"
)
...
$releaseDir = Join-Path $workspace "sd_boot\ax7020_dma_raw_copy_memory_prep_diag"
$sourceBoot = Join-Path $releaseDir "BOOT.BIN"
$sourceFsbl = Join-Path $releaseDir "fsbl.elf"
$sourceBit = Join-Path $releaseDir "dma_raw_copy_memory_prep_diag.bit"
$sourceApp = Join-Path $releaseDir "ax7020_dma_raw_copy_memory_prep_diag_app.elf"
...
foreach ($path in @($sourceBoot, $sourceFsbl, $sourceBit, $sourceApp, $sourceBootRead, $sourceReadme)) {
    Assert-Leaf -Path $path -Label "raw-copy memory-prep release artifact"
}

$bootReadText = Get-Content -Path $sourceBootRead -Raw -Encoding ASCII
foreach ($required in @("fsbl.elf", "dma_raw_copy_memory_prep_diag.bit", "ax7020_dma_raw_copy_memory_prep_diag_app.elf")) {
...
if (Test-Path -Path $targetBoot -PathType Leaf) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_before_dma_raw_copy_memory_prep_diag_$timestamp.bin"
...
Write-Host "Raw-copy memory-prep release manifest:"
...
Write-Host "Deployed raw-copy memory-prep diagnostic BOOT.BIN to SD:"
