...
param(
    ...
    [string]$Variant,
    [string]$SdDrive = "E:",
    [string]$BackupDir = "D:\FPGAhanjia\Hetero_SoC_2026_3\sdcard_backup_20260327_dma_stream_smoke_fsbl_diag"
)
...
$releaseDir = Join-Path $workspace "sd_boot\$releaseName"
...
Assert-PathType -Path $releaseDir -PathType Container -Label "FSBL diagnostic release directory"
foreach ($leafPath in @($sourceBoot, $sourceFsbl, $sourceBit, $sourceApp, $sourceBootRead, $sourceReadme)) {
    Assert-PathType -Path $leafPath -PathType Leaf -Label "FSBL diagnostic release artifact"
}

Assert-TextContains -Path $sourceBootRead -RequiredText @("fsbl.elf", "stream_smoke_dma_wrapper.bit", "ax7020_dma_raw_copy_stream_smoke_app.elf") -Label "bootgen_read.txt" | Out-Null
$requiredReadmeText = switch ($Variant) {
    "trace" { @("FSBL_DIAG AFTER_PS7_INIT", "FSBL_DIAG BEFORE_HANDOFF", "temporary board-proven Phase 3 baseline") }
    "nopostcfg" { @("FSBL_DIAG AFTER_PS7_INIT", "FSBL_DIAG BEFORE_HANDOFF", "ps7_post_config()", "temporary board-proven Phase 3 baseline") }
    "handofflite" { @("FSBL_DIAG BEFORE_POST_CONFIG", "FSBL_DIAG BEFORE_HANDOFF", "minimal-perturbation", "temporary board-proven Phase 3 baseline") }
    "handoffdelay" { @("no new UART breadcrumbs", "FsblHookBeforeHandoff()", "temporary board-proven Phase 3 baseline") }
}
Assert-TextContains -Path $sourceReadme -RequiredText $requiredReadmeText -Label "readme.txt" | Out-Null
...
$releaseArtifacts = [ordered]@{
    "BOOT.BIN"                                  = $sourceBoot
    "fsbl.elf"                                  = $sourceFsbl
    "stream_smoke_dma_wrapper.bit"              = $sourceBit
    "ax7020_dma_raw_copy_stream_smoke_app.elf"  = $sourceApp
    "bootgen_read.txt"                          = $sourceBootRead
    "readme.txt"                                = $sourceReadme
}

Write-Host "Stream-smoke FSBL diagnostic release manifest:"
...
if (Test-Path -Path $targetBoot -PathType Leaf) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_before_${releaseName}_$timestamp.bin"
...
Write-Host "Deployed stream-smoke FSBL diagnostic BOOT.BIN to SD:"
