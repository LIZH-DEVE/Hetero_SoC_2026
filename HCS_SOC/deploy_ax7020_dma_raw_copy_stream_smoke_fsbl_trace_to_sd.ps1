...
param(
    [string]$SdDrive = "E:",
    [string]$BackupDir = "D:\FPGAhanjia\Hetero_SoC_2026_3\sdcard_backup_20260327_dma_stream_smoke_fsbl_trace"
)
...
$releaseDir = Join-Path $workspace "sd_boot\ax7020_dma_raw_copy_stream_smoke_fsbl_trace"
...
Assert-PathType -Path $releaseDir -PathType Container -Label "FSBL trace release directory"
foreach ($leafPath in @($sourceBoot, $sourceFsbl, $sourceBit, $sourceApp, $sourceBootRead, $sourceReadme)) {
    Assert-PathType -Path $leafPath -PathType Leaf -Label "FSBL trace release artifact"
}
...
Assert-TextContains -Path $sourceReadme -RequiredText @("FSBL_DIAG AFTER_PS7_INIT", "experimental") -Label "readme.txt"

New-Item -ItemType Directory -Force -Path $BackupDir | Out-Null
if (Test-Path -Path $targetBoot -PathType Leaf) {
    $timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $backupBoot = Join-Path $BackupDir "BOOT_before_dma_stream_smoke_fsbl_trace_$timestamp.bin"
...
Write-Host "Deployed dedicated stream-smoke FSBL trace BOOT.BIN to SD:"
