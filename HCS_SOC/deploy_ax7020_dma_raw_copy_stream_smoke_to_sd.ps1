...
param(
    [string]$SdDrive = "E:",
    [string]$BackupDir = "D:\FPGAhanjia\Hetero_SoC_2026_3\sdcard_backup_20260327_dma_stream_smoke"
)
...
$releaseDir = Join-Path $workspace "sd_boot\ax7020_dma_raw_copy_stream_smoke"
...
Assert-PathType -Path $releaseDir -PathType Container -Label "Stream-smoke release directory"
foreach ($leafPath in @($sourceBoot, $sourceFsbl, $sourceBit, $sourceApp, $sourceBootRead, $sourceReadme)) {
    Assert-PathType -Path $leafPath -PathType Leaf -Label "Stream-smoke release artifact"
}
...
Write-Host "Stream-smoke release manifest:"
...
if (Test-Path -Path $targetBoot -PathType Leaf) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_before_dma_raw_copy_stream_smoke_$timestamp.bin"
...
Write-Host "Deployed stream-smoke BOOT.BIN to SD:"
Write-Host $targetBoot
Write-Host "SHA256: $targetHash"
Write-Host "Current status:"
Write-Host "  - this is the experimental dedicated stream-smoke with-bit image"
Write-Host "  - do not treat it as the current Phase 3 board baseline"
Write-Host "  - use ax7020_design1fsbl_stream_smoke_app as the temporary board-proven Phase 3 baseline"
