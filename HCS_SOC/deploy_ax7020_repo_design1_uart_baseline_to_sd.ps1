...
param(
    [string]$SdDrive = "E:",
    [string]$BackupDir = "D:\FPGAhanjia\Hetero_SoC_2026_3\sdcard_backup_20260324_repo_design1_uart_baseline"
)

$ErrorActionPreference = "Stop"

$workspace = Split-Path -Parent $MyInvocation.MyCommand.Path
$sourceBoot = Join-Path $workspace "sd_boot\ax7020_repo_design1_uart_baseline\BOOT.BIN"
$targetBoot = Join-Path $SdDrive "BOOT.BIN"

if (-not (Test-Path $sourceBoot)) {
    throw "Repo design_1 UART baseline BOOT.BIN not found: $sourceBoot"
}
...
if (Test-Path $targetBoot) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_from_sd_before_repo_design1_uart_baseline_$timestamp.bin"
...
Write-Host "Deployed repo design_1 UART baseline BOOT.BIN to SD:"
