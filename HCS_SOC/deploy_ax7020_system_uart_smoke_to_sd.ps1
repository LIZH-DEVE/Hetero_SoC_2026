...
param(
    [string]$SdDrive = "E:",
    [string]$BackupDir = "D:\FPGAhanjia\Hetero_SoC_2026_3\sdcard_backup_20260324_system_uart_smoke"
)

$ErrorActionPreference = "Stop"

$workspace = Split-Path -Parent $MyInvocation.MyCommand.Path
$sourceBoot = Join-Path $workspace "sd_boot\ax7020_system_uart_smoke_system\BOOT.BIN"
$targetBoot = Join-Path $SdDrive "BOOT.BIN"

if (-not (Test-Path $sourceBoot)) {
    throw "System UART smoke BOOT.BIN not found: $sourceBoot"
}
...
if (Test-Path $targetBoot) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_from_sd_before_system_uart_smoke_$timestamp.bin"
...
Write-Host "Deployed system UART smoke BOOT.BIN to SD:"
