[CmdletBinding()]
param(
    [ValidateSet("read_only", "write_ctrl_safe", "write_key_1c_flip", "write_key_18_flip", "write_key_14_flip", "write_key_10_flip")]
...
$sourceBoot = Join-Path $workspace "sd_boot\ax7020_udp_gateway_backup_xsa_ws2_smoke\$TestCase\BOOT.BIN"
$targetBoot = Join-Path $SdDrive "BOOT.BIN"

if (-not (Test-Path $sourceBoot)) {
    throw "backup-XSA ws2 smoke BOOT.BIN not found for test case '$TestCase': $sourceBoot"
}
...
if (Test-Path $targetBoot) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_from_sd_before_backup_xsa_ws2_smoke_${TestCase}_$timestamp.bin"
...
Write-Host "Deployed backup-XSA ws2 smoke BOOT.BIN to SD:"
