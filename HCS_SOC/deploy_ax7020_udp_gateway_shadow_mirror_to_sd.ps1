...
param(
    [string]$SdDrive = "E:",
    [string]$BackupDir = "D:\FPGAhanjia\Hetero_SoC_2026_3\sdcard_backup_udp_gateway_shadow_mirror"
)
...
$sourceBit = Join-Path $releaseDir "udp_gateway_shadow_mirror_wrapper.bit"
$sourceApp = Join-Path $releaseDir "ax7020_udp_gateway_shadow_mirror_app.elf"
$sdDriveRoot = Resolve-SdDriveRoot -ConfiguredDrive $SdDrive
$targetBoot = Join-Path $sdDriveRoot "BOOT.BIN"

Assert-PathType -Path $releaseDir -PathType Container -Label "UDP gateway shadow mirror release directory"
foreach ($leafPath in @($sourceBoot, $sourceBif, $sourceReadme, $sourceBootgenRead, $sourceFsbl, $sourceBit, $sourceApp)) {
    Assert-PathType -Path $leafPath -PathType Leaf -Label "UDP gateway shadow mirror release artifact"
}
...
Write-Host "UDP gateway shadow mirror release manifest:"
...
if (Test-Path -Path $targetBoot -PathType Leaf) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_before_udp_gateway_shadow_mirror_$timestamp.bin"
...
Write-Host "Deployed UDP gateway shadow mirror BOOT.BIN to SD:"
