...
param(
    [string]$SdDrive = "E:",
    [string]$BackupDir = "D:\FPGAhanjia\Hetero_SoC_2026_3\sdcard_backup_20260324_vendor_ps_uart"
)

$ErrorActionPreference = "Stop"

$workspace = Split-Path -Parent $MyInvocation.MyCommand.Path
$sourceBoot = Join-Path $workspace "sd_boot\ax7020_vendor_ps_uart_baseline\BOOT.BIN"
$targetBoot = Join-Path $SdDrive "BOOT.BIN"

if (-not (Test-Path $sourceBoot)) {
    throw "Vendor ps_uart BOOT.BIN not found: $sourceBoot"
}
...
if (Test-Path $targetBoot) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_from_sd_before_vendor_ps_uart_$timestamp.bin"
...
Write-Host "Deployed vendor ps_uart baseline BOOT.BIN to SD:"
