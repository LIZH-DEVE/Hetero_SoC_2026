...
$sourceBoot = Join-Path $workspace "sd_boot\crypto_direct_write_smoke\$TestCase\BOOT.BIN"
$targetBoot = Join-Path $SdDrive "BOOT.BIN"

if (-not (Test-Path $sourceBoot)) {
    throw "Smoke BOOT.BIN not found for test case '$TestCase': $sourceBoot"
}
...
if (Test-Path $targetBoot) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_from_sd_before_crypto_direct_write_smoke_${TestCase}_$timestamp.bin"
...
Write-Host "Deployed smoke BOOT.BIN to SD:"
