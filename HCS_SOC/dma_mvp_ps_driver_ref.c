...
static uint64_t dma_ring_read_counter64(const dma_ring_ctx_t *ctx, uint32_t low_offset, uint32_t high_offset)
{
    uint32_t low_word;
    uint32_t high_word;
    ...
    low_word = Xil_In32((UINTPTR)(ctx->csr_base + low_offset));
    high_word = Xil_In32((UINTPTR)(ctx->csr_base + high_offset));
...
void dma_ring_counter_clear(const dma_ring_ctx_t *ctx)
{
    if (ctx != 0) {
        Xil_Out32((UINTPTR)(ctx->csr_base + DMA_CSR_COUNTER_CTRL), DMA_COUNTER_CTRL_CLEAR_ALL);
        DATA_SYNC;
    }
}

void dma_ring_counter_snapshot(const dma_ring_ctx_t *ctx)
{
    if (ctx != 0) {
        Xil_Out32((UINTPTR)(ctx->csr_base + DMA_CSR_COUNTER_CTRL), DMA_COUNTER_CTRL_SNAPSHOT);
        DATA_SYNC;
    }
}

int dma_ring_read_counter_snapshot(const dma_ring_ctx_t *ctx, dma_counter_snapshot_t *out)
{
    if (ctx == 0 || out == 0) {
        return -1;
    }

    out->rollback_event_count = dma_ring_read_counter64(ctx, DMA_CSR_ROLLBACK_EVENT_COUNT_LO, DMA_CSR_ROLLBACK_EVENT_COUNT_HI);
    out->recovery_active_cycles = dma_ring_read_counter64(ctx, DMA_CSR_RECOVERY_ACTIVE_CYCLES_LO, DMA_CSR_RECOVERY_ACTIVE_CYCLES_HI);
    out->recovery_last_window_cycles = dma_ring_read_counter64(ctx, DMA_CSR_RECOVERY_LAST_WINDOW_CYCLES_LO, DMA_CSR_RECOVERY_LAST_WINDOW_CYCLES_HI);
    out->recovery_max_window_cycles = dma_ring_read_counter64(ctx, DMA_CSR_RECOVERY_MAX_WINDOW_CYCLES_LO, DMA_CSR_RECOVERY_MAX_WINDOW_CYCLES_HI);
    out->error_qualified_packet_count = dma_ring_read_counter64(ctx, DMA_CSR_ERROR_QUALIFIED_PACKET_COUNT_LO, DMA_CSR_ERROR_QUALIFIED_PACKET_COUNT_HI);
    out->backend_total_cycles = dma_ring_read_counter64(ctx, DMA_CSR_BACKEND_TOTAL_CYCLES_LO, DMA_CSR_BACKEND_TOTAL_CYCLES_HI);
    out->backend_accept_cycles = dma_ring_read_counter64(ctx, DMA_CSR_BACKEND_ACCEPT_CYCLES_LO, DMA_CSR_BACKEND_ACCEPT_CYCLES_HI);
    out->backend_starvation_cycles = dma_ring_read_counter64(ctx, DMA_CSR_BACKEND_STARVATION_CYCLES_LO, DMA_CSR_BACKEND_STARVATION_CYCLES_HI);
    out->high_water_count = dma_ring_read_counter64(ctx, DMA_CSR_HIGH_WATER_COUNT_LO, DMA_CSR_HIGH_WATER_COUNT_HI);
    out->drop_pulse_count = dma_ring_read_counter64(ctx, DMA_CSR_DROP_PULSE_COUNT_LO, DMA_CSR_DROP_PULSE_COUNT_HI);
    return 0;
}
