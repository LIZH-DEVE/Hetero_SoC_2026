...
function Copy-ExistingArtifactVariants {
    param(
        [Parameter(Mandatory = $true)]
        [string]$SourceDir,
        [Parameter(Mandatory = $true)]
    ...
    foreach ($artifact in $Artifacts) {
        if ($artifact -is [string]) {
            ...
            $candidateNames = @($artifact)
        }
        else {
            $artifactName = [string]$artifact.Name
            $candidateNames = @($artifact.Candidates)
        }
        ...
        foreach ($candidateName in $candidateNames) {
            ...
            $sourcePath = Join-Path $SourceDir $candidateName
            if (Test-Path $sourcePath) {
                Copy-Item -LiteralPath $sourcePath -Destination (Join-Path $DestinationDir $artifactName) -Force
                break
            }
        }
    }
...
if ([string]::IsNullOrWhiteSpace($CheckpointPath)) {
    $CheckpointPath = Resolve-ExistingPath -Candidates @(
        ...
        (Join-Path $ImplRunDir "design_1_wrapper_postroute_physopt.dcp"),
        (Join-Path $ImplRunDir "design_1_wrapper_routed.dcp")
...
$rawImplArtifacts = @(
    "runme.log",
    @{ Name = "udp_gateway_shadow_mirror_wrapper.bit"; Candidates = @("udp_gateway_shadow_mirror_wrapper.bit", "design_1_wrapper.bit") },
    @{ Name = "udp_gateway_shadow_mirror_wrapper_routed.dcp"; Candidates = @("udp_gateway_shadow_mirror_wrapper_routed.dcp", "design_1_wrapper_routed.dcp") },
    @{ Name = "udp_gateway_shadow_mirror_wrapper_postroute_physopt.dcp"; Candidates = @("udp_gateway_shadow_mirror_wrapper_postroute_physopt.dcp", "design_1_wrapper_postroute_physopt.dcp") },
    @{ Name = "udp_gateway_shadow_mirror_wrapper_utilization_placed.rpt"; Candidates = @("udp_gateway_shadow_mirror_wrapper_utilization_placed.rpt", "design_1_wrapper_utilization_placed.rpt") },
    @{ Name = "udp_gateway_shadow_mirror_wrapper_timing_summary_routed.rpt"; Candidates = @("udp_gateway_shadow_mirror_wrapper_timing_summary_routed.rpt", "design_1_wrapper_timing_summary_routed.rpt") },
    @{ Name = "udp_gateway_shadow_mirror_wrapper_timing_summary_postroute_physopted.rpt"; Candidates = @("udp_gateway_shadow_mirror_wrapper_timing_summary_postroute_physopted.rpt", "design_1_wrapper_timing_summary_postroute_physopted.rpt") },
    @{ Name = "udp_gateway_shadow_mirror_wrapper_power_routed.rpt"; Candidates = @("udp_gateway_shadow_mirror_wrapper_power_routed.rpt", "design_1_wrapper_power_routed.rpt") },
    @{ Name = "udp_gateway_shadow_mirror_wrapper_clock_utilization_routed.rpt"; Candidates = @("udp_gateway_shadow_mirror_wrapper_clock_utilization_routed.rpt", "design_1_wrapper_clock_utilization_routed.rpt") },
    @{ Name = "udp_gateway_shadow_mirror_wrapper_route_status.rpt"; Candidates = @("udp_gateway_shadow_mirror_wrapper_route_status.rpt", "design_1_wrapper_route_status.rpt") },
    @{ Name = "udp_gateway_shadow_mirror_wrapper_drc_routed.rpt"; Candidates = @("udp_gateway_shadow_mirror_wrapper_drc_routed.rpt", "design_1_wrapper_drc_routed.rpt") },
    @{ Name = "udp_gateway_shadow_mirror_wrapper_methodology_drc_routed.rpt"; Candidates = @("udp_gateway_shadow_mirror_wrapper_methodology_drc_routed.rpt", "design_1_wrapper_methodology_drc_routed.rpt") },
    @{ Name = "udp_gateway_shadow_mirror_wrapper_io_placed.rpt"; Candidates = @("udp_gateway_shadow_mirror_wrapper_io_placed.rpt", "design_1_wrapper_io_placed.rpt") },
    @{ Name = "udp_gateway_shadow_mirror_wrapper_control_sets_placed.rpt"; Candidates = @("udp_gateway_shadow_mirror_wrapper_control_sets_placed.rpt", "design_1_wrapper_control_sets_placed.rpt") },
    @{ Name = "udp_gateway_shadow_mirror_wrapper_bus_skew_routed.rpt"; Candidates = @("udp_gateway_shadow_mirror_wrapper_bus_skew_routed.rpt", "design_1_wrapper_bus_skew_routed.rpt") },
    @{ Name = "udp_gateway_shadow_mirror_wrapper_bus_skew_postroute_physopted.rpt"; Candidates = @("udp_gateway_shadow_mirror_wrapper_bus_skew_postroute_physopted.rpt", "design_1_wrapper_bus_skew_postroute_physopted.rpt") }
)
$releaseArtifacts = @(
    "BOOT.BIN",
    "bootgen_read.txt",
...
Copy-ExistingArtifactVariants -SourceDir $ImplRunDir -DestinationDir $rawImplDir -Artifacts $rawImplArtifacts
...
foreach ($candidate in @(
    ...
    if (Test-Path $candidate) {
        $pblockSummary = (Resolve-Path $candidate).Path
        break
    }
}
...
$pblockCount = if ($pblockSummary) { Get-PblockCount -SummaryPath $pblockSummary } else { 0 }
...
$resolvedManifestPath = Join-Path $OutputDir "engineering_evidence_manifest.json"
$resolvedSummaryPath = Join-Path $OutputDir "engineering_evidence_summary.md"
...
Set-Content -Path $resolvedSummaryPath -Value $summaryLines -Encoding utf8
