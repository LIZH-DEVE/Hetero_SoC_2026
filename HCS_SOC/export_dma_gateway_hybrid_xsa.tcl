...
set out_xsa [file join $workspace_root "dma_gateway_hybrid_wrapper.xsa"]
...
if {[info exists ::env(DMA_GATEWAY_HYBRID_EXPORT_DRY_RUN)] && $::env(DMA_GATEWAY_HYBRID_EXPORT_DRY_RUN) eq "1"} {
    set dry_run 1
}
...
if {![file exists $raw_bd]} {
    error "hybrid BD clone was not created: $raw_bd"
}
if {[llength [get_files -quiet $raw_bd]] == 0} {
...
connect_bd_net [get_bd_pins dma_gateway_hybrid_0/clk] [get_bd_pins processing_system7_0/FCLK_CLK0]
connect_bd_net [get_bd_pins dma_gateway_hybrid_0/rst_n] [get_bd_pins rst_ps7_0_100M/peripheral_aresetn]
...
connect_bd_intf_net [get_bd_intf_pins dma_gateway_hybrid_0/m_axi_dma_wr] [get_bd_intf_pins axi_mem_intercon/S00_AXI]
connect_bd_intf_net [get_bd_intf_pins dma_gateway_hybrid_0/m_axi_fetcher] [get_bd_intf_pins axi_mem_intercon/S01_AXI]
connect_bd_intf_net [get_bd_intf_pins dma_gateway_hybrid_0/m_axi_s2mm] [get_bd_intf_pins axi_mem_intercon/S02_AXI]

assign_bd_address -offset 0x40000000 -range 4K \
    -target_address_space [get_bd_addr_spaces processing_system7_0/Data] \
    [get_bd_addr_segs dma_gateway_hybrid_0/s_axil_ctrl/reg0] -force
assign_bd_address -offset 0x40001000 -range 4K \
    -target_address_space [get_bd_addr_spaces processing_system7_0/Data] \
    [get_bd_addr_segs dma_gateway_hybrid_0/s_axil_dma/reg0] -force
...
foreach addr_space {m_axi_dma_wr m_axi_fetcher m_axi_s2mm} {
    assign_bd_address -offset 0x00000000 -range 512M \
        -target_address_space [get_bd_addr_spaces dma_gateway_hybrid_0/$addr_space] \
...
update_compile_order -fileset sources_1
set_property top dma_gateway_hybrid_wrapper [current_fileset]

if {$dry_run} {
    puts "Dry-run complete. Hybrid gateway hardware graph and wrapper generated without synth/impl."
    close_project
    return
}
