...
function Get-PresentPnpDevicesFromPnputil {
    ...
    foreach ($rawLine in ($output -split "`r?`n")) {
        ...
        if ([string]::IsNullOrWhiteSpace($line)) {
            if ($current.Contains("InstanceId")) {
                $records += New-FtdiDeviceRecord `
                    -Status ([string]$current["Status"]) `
                    -Class ([string]$current["Class"]) `
                    -FriendlyName ([string]$current["FriendlyName"]) `
        ...
        if ($line -match '^\s*Device Description:\s*(.+)$') {
            $current["FriendlyName"] = $matches[1].Trim()
            continue
        }
...
function Get-PresentPnpDevices {
    try {
        $pnpDevices = @(Get-PnpDevice -PresentOnly -ErrorAction Stop | ForEach-Object {
            Convert-PnpDeviceToDeviceRecord -Device $_
        })
        if ($pnpDevices.Count -gt 0) {
            return @($pnpDevices | Sort-Object InstanceId)
        }
    } catch {
    }

    try {
        $cimDevices = @(Get-CimInstance Win32_PnPEntity -ErrorAction Stop | ForEach-Object {
            Convert-CimPnPEntityToDeviceRecord -Device $_
        })
        if ($cimDevices.Count -gt 0) {
            return @($cimDevices | Sort-Object InstanceId)
        }
    } catch {
    }

    return @(Get-PresentPnpDevicesFromPnputil | Sort-Object InstanceId)
}

function Get-FtdiUsbDevicesFromPnputil {
    return @(Get-PresentPnpDevicesFromPnputil | Where-Object {
        $_.InstanceId -match '^USB\\VID_0403&PID_[0-9A-F]{4}'
    } | Sort-Object InstanceId)
}

function Get-FtdiUsbDevices {
    $devices = Get-PresentPnpDevices | Where-Object {
        $_.InstanceId -match '^USB\\VID_0403&PID_[0-9A-F]{4}'
    } | Sort-Object InstanceId
...
function Resolve-FtdiUserAreaRepairScript {
    $candidates = @(
        (Join-Path $PSScriptRoot "repair_ax7020_ftdi_user_area.py"),
        (Join-Path (Get-WorkspaceRoot) "repair_ax7020_ftdi_user_area.py"),
        (Join-Path (Get-WorkspaceRoot) "scripts\repair_ax7020_ftdi_user_area.py")
    )
...
function New-FtdiRuntimeDirectory {
    ...
    $copyMap = @(
        ...
        @{ Source = Join-Path $VivadoRoot "lib\win64.o\tcl86t.dll"; Name = "tcl85t.dll" },
...
function Get-PnpDriverSnapshot {
    ...
    try {
        $driver = Get-CimInstance Win32_PnPSignedDriver -ErrorAction Stop | Where-Object {
    ...
    return Get-PnpDriverSnapshotFromPnputil -InstanceId $InstanceId
}
...
function Get-PnpDriverSnapshotFromPnputil {
    ...
    $output = (& pnputil /enum-devices /instanceid "$InstanceId" /drivers 2>&1 | Out-String)
    ...
    foreach ($rawLine in ($output -split "`r?`n")) {
        $line = $rawLine.TrimEnd()
        if ($line -match '^\s*Device Description:\s*(.+)$') {
            $map["DeviceName"] = $matches[1].Trim()
            continue
        }
        if ($line -match '^\s*Manufacturer Name:\s*(.+)$') {
            $map["Manufacturer"] = $matches[1].Trim()
            continue
        }
        if ($line -match '^\s*Provider Name:\s*(.+)$' -and -not $map.Contains("DriverProviderName")) {
            $map["DriverProviderName"] = $matches[1].Trim()
            continue
        }
        if ($line -match '^\s*Driver Version:\s*(.+)$' -and -not $map.Contains("DriverVersion")) {
            $map["DriverVersion"] = $matches[1].Trim()
            continue
        }
        if ($line -match '^\s*Driver Name:\s*(.+)$' -and -not $map.Contains("InfName")) {
            $map["InfName"] = $matches[1].Trim()
            continue
        }
    }
    ...
    [PSCustomObject]@{
        ...
        DriverProviderName = [string]$map["DriverProviderName"]
        DriverVersion      = [string]$map["DriverVersion"]
        InfName            = [string]$map["InfName"]
    }
}
