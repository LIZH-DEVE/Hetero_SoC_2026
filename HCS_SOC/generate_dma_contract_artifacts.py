...
def render_c_header(manifest: dict) -> str:
    ...
    counter_ctrl_bits = manifest["csr"].get("counter_ctrl_bits", {})
    ...
    lines.append("")
    for name, bit in counter_ctrl_bits.items():
        lines.append(f"#define {_macro_name('DMA_COUNTER_CTRL_BIT', name)} {bit}u")
        lines.append(
            f"#define {_macro_name('DMA_COUNTER_CTRL', name)} (1u << {_macro_name('DMA_COUNTER_CTRL_BIT', name)})"
        )
    if counter_ctrl_bits:
        lines.append("")
...
def render_sv_package(manifest: dict) -> str:
    ...
    lines.append("")
    for name, bit in counter_ctrl_bits.items():
        lines.append(f"  localparam int unsigned DMA_COUNTER_CTRL_BIT_{name} = {bit};")
        lines.append(
            f"  localparam logic [31:0] DMA_COUNTER_CTRL_{name} = 32'h00000001 << DMA_COUNTER_CTRL_BIT_{name};"
        )
    if counter_ctrl_bits:
        lines.append("")
