...
#define SOFT_RESET_RING_COUNT 1u

static XUartPs g_uart;
static dma_ring_desc_t g_ring[SOFT_RESET_RING_COUNT]
...
int main(void)
{
    ...
    dma_ring_init(&ctx,
                  ...
                  SOFT_RESET_RING_COUNT);

    xil_printf("RAWCOPY SOFT RESET DIAG\r\n");
    xil_printf("RAWCOPY_SOFT_RESET_STAGE DRIVER_INIT_DONE csr=0x%08lx ring=0x%08lx count=%u\r\n",
               (unsigned long)ctx.csr_base,
               (unsigned long)ctx.ring_base_phys,
               (unsigned)ctx.ring_size);
    xil_printf("RAWCOPY_SOFT_RESET_STAGE BEFORE_SOFT_RESET\r\n");

    dma_ring_soft_reset(&ctx);

    xil_printf("RAWCOPY_SOFT_RESET_STAGE AFTER_SOFT_RESET\r\n");
    xil_printf("RAWCOPY_SOFT_RESET_OK\r\n");

    for (;;) {
        busy_wait();
        xil_printf("RAWCOPY_SOFT_RESET_HEARTBEAT %u\r\n", heartbeat++);
    }
}
