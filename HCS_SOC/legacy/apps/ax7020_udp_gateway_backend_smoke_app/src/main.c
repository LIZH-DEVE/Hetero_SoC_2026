...
#define BACKEND_CTRL_BASEADDR 0x40000000u
#define BACKEND_DMA_BASEADDR 0x40001000u
...
#define BACKEND_LOCAL_IP 0xC0A80114u
#define BACKEND_LOCAL_MAC_LO 0x35000120u
#define BACKEND_LOCAL_MAC_HI 0x0000020Au

#define BACKEND_RING_ENTRY_COUNT 2u
#define BACKEND_MAX_PAYLOAD_BYTES 1024u
#define BACKEND_SHORT_BYTES 64u
#define BACKEND_OVERFLOW_BYTES 128u
#define BACKEND_OVERFLOW_CAPACITY 64u
#define BACKEND_WORD_BYTES 4u
#define BACKEND_POLL_TIMEOUT 3000000u

#define BACKEND_PORT_AES 4660u
#define BACKEND_PORT_SM4 4661u
#define BACKEND_PORT_CTRL 4662u
#define BACKEND_PORT_WRONG 4663u

typedef struct {
    dma_ring_desc_t ring[BACKEND_RING_ENTRY_COUNT];
    uint8_t guard[DMA_CACHELINE_BYTES];
} backend_desc_region_t;

typedef struct {
    uint32_t words[BACKEND_MAX_PAYLOAD_BYTES / BACKEND_WORD_BYTES];
    uint8_t guard[DMA_CACHELINE_BYTES];
} backend_buffer_region_t;

static backend_desc_region_t g_desc_region
    __attribute__((aligned(DMA_ALIGNMENT_BYTES), section(".dma_desc_region")));
static backend_buffer_region_t g_dst_region
...
{
    Xil_Out32((UINTPTR)(BACKEND_CTRL_BASEADDR + offset), value);
}

static inline uint32_t wrap_read32(uint32_t offset)
{
    return Xil_In32((UINTPTR)(BACKEND_CTRL_BASEADDR + offset));
}
...
static int wait_for_done_csw(volatile dma_ring_desc_t *desc, uint32_t *csw_out)
{
    ...
    for (i = 0; i < BACKEND_POLL_TIMEOUT; ++i) {
        uint32_t csw;
...
static void configure_backend_control(void)
{
    wrap_write32(WRAP_REG_NET_CFG0, NET_CFG_ENABLE | NET_CFG_INJECT_SEL | NET_CFG_ARP_ENABLE);
    wrap_write32(WRAP_REG_NET_LOCAL_IP, BACKEND_LOCAL_IP);
    wrap_write32(WRAP_REG_NET_LOCAL_MAC_LO, BACKEND_LOCAL_MAC_LO);
    wrap_write32(WRAP_REG_NET_LOCAL_MAC_HI, BACKEND_LOCAL_MAC_HI);
    wrap_write32(WRAP_REG_INJ_CTRL, 1u);
    wrap_write32(WRAP_REG_TXCAP_CTRL, 1u);
}

static int backend_inject_udp_frame(uint16_t dst_port, uint16_t payload_len_bytes)
{
    uint32_t frame_words[11 + (BACKEND_MAX_PAYLOAD_BYTES / BACKEND_WORD_BYTES)];
    uint32_t payload_words;
    uint32_t frame_word_count;
    uint32_t word_idx;

    if ((payload_len_bytes == 0u) || (payload_len_bytes > BACKEND_MAX_PAYLOAD_BYTES)) {
        return -5;
    }
    if ((payload_len_bytes & (BACKEND_WORD_BYTES - 1u)) != 0u) {
        return -6;
    }

    payload_words = payload_len_bytes / BACKEND_WORD_BYTES;
    ...
    frame_words[2] = BACKEND_LOCAL_MAC_LO;
    ...
    frame_words[8] = BACKEND_LOCAL_IP;
...
{
    uint32_t word_idx;
    uint32_t expected_words = expected_bytes / BACKEND_WORD_BYTES;
...
static int run_dma_probe_case(const char *case_name,
                              uint16_t dst_port,
                              ...
                              uint32_t expect_sts)
{
    ...
    rc = dma_ring_submit_stream(&ctx,
                                ...
                                0);
    if (rc != 0) {
        xil_printf("DMA_PROBE %s submit rc=%d\r\n", case_name, rc);
        return -10;
    }

    xil_printf("DMA_PROBE %s PREP capacity=%u packet_len=%u\r\n",
               case_name,
               (unsigned int)capacity_bytes,
               (unsigned int)packet_len_bytes);

    rc = backend_inject_udp_frame(dst_port, (uint16_t)packet_len_bytes);
    if (rc != 0) {
        xil_printf("DMA_PROBE %s inject rc=%d\r\n", case_name, rc);
        return -11;
    }

    rc = wait_for_done_csw(&g_desc_region.ring[0], &csw);
    if (rc != 0) {
        xil_printf("DMA_PROBE %s csw timeout\r\n", case_name);
        return -12;
    }

    actual_len = dma_ring_read_actual_len(&g_desc_region.ring[0]);
    if (actual_len != expected_actual_len) {
        xil_printf("DMA_PROBE %s actual_len mismatch exp=%u got=%u\r\n",
                   case_name,
    ...
    if (((csw & DMA_DESC_CSW_ERR) != 0u) != (expect_err != 0u)) {
        xil_printf("DMA_PROBE %s err flag mismatch csw=0x%08x\r\n", case_name, (unsigned int)csw);
        return -14;
    }
    if ((csw & DMA_DESC_CSW_STS_MASK) != expect_sts) {
        xil_printf("DMA_PROBE %s sts mismatch exp=%s got=%s\r\n",
                   case_name,
    ...
    xil_printf("DMA_PROBE %s PASS actual_len=%u\r\n",
               case_name,
               (unsigned int)actual_len);
    return 0;
}
...
static int run_wrong_port_case(void)
{
    ...
    rc = backend_inject_udp_frame(BACKEND_PORT_WRONG, BACKEND_SHORT_BYTES);
    if (rc != 0) {
        return -1;
    }
    ...
    xil_printf("DMA_PROBE WRONG_PORT PASS count=%u\r\n",
...
static int run_unaligned_reject_case(void)
{
    ...
    rc = backend_inject_udp_frame(BACKEND_PORT_AES, 66u);
    if (rc != -6) {
        return -1;
    }
    ...
    xil_printf("DMA_PROBE UNALIGNED_REJECT PASS rc=%d\r\n", rc);
    return 0;
}
...
static int run_direct_backend_regression(void)
{
    ...
    xil_printf("DIRECT_BACKEND START\r\n");
    rc = udp_crypto_gateway_run_backend_split_smoke_checked();
    if (rc != 0) {
        return rc;
    }
    xil_printf("DIRECT_BACKEND PASS\r\n");
    return 0;
}
...
static int run_dma_probe_smoke(void)
{
    ...
    rc = run_dma_probe_case("EXACT_FIT",
                            BACKEND_PORT_AES,
                            1024u,
                            1024u,
                            1024u,
                            ...
                            DMA_DESC_CSW_STS_OK);
    if (rc != 0) {
        xil_printf("DMA_PROBE EXACT_FIT FAIL rc=%d\r\n", rc);
        return 1;
    }

    rc = run_dma_probe_case("SHORT",
                            BACKEND_PORT_SM4,
                            1024u,
                            BACKEND_SHORT_BYTES,
                            BACKEND_SHORT_BYTES,
                            ...
                            DMA_DESC_CSW_STS_OK);
    if (rc != 0) {
        xil_printf("DMA_PROBE SHORT FAIL rc=%d\r\n", rc);
        return 2;
    }

    rc = run_dma_probe_case("OVERFLOW",
                            BACKEND_PORT_AES,
                            BACKEND_OVERFLOW_CAPACITY,
                            BACKEND_OVERFLOW_BYTES,
                            BACKEND_OVERFLOW_CAPACITY,
                            1u,
                            DMA_DESC_CSW_STS_OVERFLOW_OR_MISSING_TLAST);
    if (rc != 0) {
        xil_printf("DMA_PROBE OVERFLOW FAIL rc=%d\r\n", rc);
        return 3;
    }

    rc = run_wrong_port_case();
    if (rc != 0) {
        xil_printf("DMA_PROBE WRONG_PORT FAIL rc=%d\r\n", rc);
        return 4;
    }

    rc = run_unaligned_reject_case();
    if (rc != 0) {
        xil_printf("DMA_PROBE UNALIGNED_REJECT FAIL rc=%d\r\n", rc);
        return 5;
    }

    xil_printf("DMA_PROBE PASS\r\n");
    return 0;
}
...
int main(void)
{
    ...
    configure_backend_control();

    xil_printf("UDP gateway backend smoke image\r\n");
    xil_printf("  ctrl_base    = 0x%08x\r\n", (unsigned int)BACKEND_CTRL_BASEADDR);
    xil_printf("  dma_base     = 0x%08x\r\n", (unsigned int)BACKEND_DMA_BASEADDR);
    ...
    rc = run_direct_backend_regression();
    if (rc != 0) {
        xil_printf("DIRECT_BACKEND FAIL rc=%d\r\n", rc);
        xil_printf("UDP gateway backend smoke FAIL\r\n");
        return 1;
    }

    rc = run_dma_probe_smoke();
    if (rc != 0) {
        xil_printf("UDP gateway backend smoke FAIL\r\n");
        return 2;
    }

    xil_printf("UDP gateway backend smoke PASS\r\n");
    return 0;
}
