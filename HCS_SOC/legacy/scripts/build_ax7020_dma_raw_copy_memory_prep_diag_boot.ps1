...
$appBuildScript = Join-Path $workspace "build_ax7020_dma_raw_copy_memory_prep_diag_app.ps1"
$templateBif = Join-Path $workspace "sd_boot\ax7020_dma_raw_copy_memory_prep_diag\boot.bif"
...
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $OutputDir = Join-Path $workspace "sd_boot\ax7020_dma_raw_copy_memory_prep_diag"
}
$appBuildDir = Join-Path $workspace "ax7020_dma_raw_copy_memory_prep_diag_app\build"
...
foreach ($path in $requiredInputs) {
    if (-not (Test-Path $path)) {
        throw "Required raw-copy memory-prep build input not found: $path"
    }
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Raw-copy memory-prep diagnostic app build failed"
}

$appElf = Join-Path $appBuildDir "ax7020_dma_raw_copy_memory_prep_diag_app.elf"
foreach ($path in @($fsblElf, $bitstream.FullName, $appElf)) {
    if (-not (Test-Path $path)) {
        throw "Required raw-copy memory-prep boot artifact not found: $path"
    }
}
...
$stagedBit = Join-Path $OutputDir "dma_raw_copy_memory_prep_diag.bit"
$stagedApp = Join-Path $OutputDir "ax7020_dma_raw_copy_memory_prep_diag_app.elf"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "bootgen failed for raw-copy memory-prep diagnostic BOOT.BIN"
    }

    $readback = & $bootgen -read $bootBin 2>&1 | Out-String
    if ($LASTEXITCODE -ne 0) {
        throw "bootgen -read failed for raw-copy memory-prep diagnostic BOOT.BIN"
    }
...
Write-Host "Generated raw-copy memory-prep diagnostic BOOT.BIN:"
Write-Host $bootBin
