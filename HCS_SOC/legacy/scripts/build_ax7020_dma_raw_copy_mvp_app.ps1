...
$appSrcDir = Join-Path $workspace "ax7020_dma_raw_copy_mvp_app\src"
$buildDir = Join-Path $workspace "ax7020_dma_raw_copy_mvp_app\build"
...
$elfPath = Join-Path $buildDir "ax7020_dma_raw_copy_mvp_app.elf"
$mapPath = Join-Path $buildDir "ax7020_dma_raw_copy_mvp_app.map"
$libs = @()
...
& $gcc @linkArgs
if ($LASTEXITCODE -ne 0) {
    throw "Link failed for raw-copy MVP app"
}
...
Write-Host "Built raw-copy MVP app ELF:"
Write-Host $elfPath
