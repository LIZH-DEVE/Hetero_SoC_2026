...
$appSrcDir = Join-Path $workspace "ax7020_dma_raw_copy_stream_smoke_app\src"
$buildDir = Join-Path $workspace "ax7020_dma_raw_copy_stream_smoke_app\build"
...
function Resolve-PlatformApiBspRoot {
    ...
    throw "No dedicated stream-smoke platform API BSP root found under $WorkspaceRoot"
}
...
function Resolve-PlatformToolchainRoot {
    ...
    $candidates = @(
        (Join-Path $WorkspaceRoot "ax7020_dma_stream_smoke_platform\zynq_fsbl\zynq_fsbl_bsp\ps7_cortexa9_0"),
        (Join-Path $WorkspaceRoot "standalone_bsp\ps7_cortexa9_0")
    )
    ...
    throw "No stream-smoke toolchain BSP root with libxil.a found under $WorkspaceRoot"
}
...
if ([string]::IsNullOrWhiteSpace($PlatformSwDir)) {
    $PlatformSwDir = Resolve-PlatformApiBspRoot -WorkspaceRoot (Join-Path $workspace "ax7020_dma_stream_smoke_platform_xsct\workspace")
}
...
$elfPath = Join-Path $buildDir "ax7020_dma_raw_copy_stream_smoke_app.elf"
$mapPath = Join-Path $buildDir "ax7020_dma_raw_copy_stream_smoke_app.map"
$libs = @()
...
& $gcc @linkArgs
if ($LASTEXITCODE -ne 0) {
    throw "Link failed for stream-smoke app"
}
...
Write-Host "Built stream-smoke app ELF:"
Write-Host $elfPath
