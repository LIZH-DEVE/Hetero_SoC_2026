...
$appSrcDir = Join-Path $workspace "ax7020_dma_raw_copy_submit_diag_app\src"
...
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $OutputDir = Join-Path $workspace "ax7020_dma_raw_copy_submit_diag_app\build"
}
...
$elfPath = Join-Path $buildDir "ax7020_dma_raw_copy_submit_diag_app.elf"
$mapPath = Join-Path $buildDir "ax7020_dma_raw_copy_submit_diag_app.map"
...
if ($LASTEXITCODE -ne 0) {
    throw "Compile failed for raw-copy submit diagnostic main.c"
}
...
& $gcc @linkArgs
if ($LASTEXITCODE -ne 0) {
    throw "Link failed for raw-copy submit diagnostic app"
}
...
Write-Host "Built raw-copy submit diagnostic app ELF:"
Write-Host $elfPath
