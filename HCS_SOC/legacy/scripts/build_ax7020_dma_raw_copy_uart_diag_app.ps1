...
$appSrcDir = Join-Path $workspace "ax7020_dma_raw_copy_uart_diag_app\src"
$mainSource = Join-Path $appSrcDir "main.c"
$linkerScript = Join-Path $appSrcDir "lscript.ld"

if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $OutputDir = Join-Path $workspace "ax7020_dma_raw_copy_uart_diag_app\build"
}
...
$elfPath = Join-Path $buildDir "ax7020_dma_raw_copy_uart_diag_app.elf"
$mapPath = Join-Path $buildDir "ax7020_dma_raw_copy_uart_diag_app.map"
...
if ($LASTEXITCODE -ne 0) {
    throw "Compile failed for UART-only raw-copy diag main.c"
}
...
& $gcc @linkArgs
if ($LASTEXITCODE -ne 0) {
    throw "Link failed for UART-only raw-copy diag app"
}
...
Write-Host "Built UART-only raw-copy diagnostic app ELF:"
Write-Host $elfPath
