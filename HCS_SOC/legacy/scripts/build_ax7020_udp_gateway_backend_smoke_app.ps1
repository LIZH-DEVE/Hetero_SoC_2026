...
$appSrcDir = Join-Path $workspace "ax7020_udp_gateway_backend_smoke_app\src"
$buildDir = Join-Path $workspace "ax7020_udp_gateway_backend_smoke_app\build"
...
function Resolve-BackendApiBspRoot {
    ...
    throw "No backend smoke platform API BSP root found under $WorkspaceRoot. Run build_ax7020_udp_gateway_backend_smoke_boot.ps1 first or pass -PlatformSwDir explicitly."
}

function Resolve-BackendToolchainRoot {
    param(
        [string]$WorkspaceRoot,
        [string]$ApiBspRoot,
        [string]$PlatformName
    ...
    throw "No backend smoke toolchain BSP root with libxil.a found under $WorkspaceRoot"
}
...
if ([string]::IsNullOrWhiteSpace($PlatformSwDir)) {
    $PlatformSwDir = Resolve-BackendApiBspRoot -WorkspaceRoot $defaultWorkspaceRoot -PlatformName $defaultPlatformName
}

$workspaceRoot = Resolve-WorkspaceRootFromApiBspRoot -ApiBspRoot $PlatformSwDir
$toolchainRoot = Resolve-BackendToolchainRoot -WorkspaceRoot $workspaceRoot -ApiBspRoot $PlatformSwDir -PlatformName $defaultPlatformName
...
$includeDirs = @($includeDir, $appSrcDir, $gatewaySrcDir, $workspace)
...
$elfPath = Join-Path $buildDir "ax7020_udp_gateway_backend_smoke_app.elf"
$mapPath = Join-Path $buildDir "ax7020_udp_gateway_backend_smoke_app.map"
$libs = @()
...
if ($LASTEXITCODE -ne 0) {
    throw "Compile failed for backend smoke main.c"
}
...
& $gcc @linkArgs
if ($LASTEXITCODE -ne 0) {
    throw "Link failed for backend smoke app"
}
...
Write-Host "Built backend smoke app ELF:"
Write-Host $elfPath
