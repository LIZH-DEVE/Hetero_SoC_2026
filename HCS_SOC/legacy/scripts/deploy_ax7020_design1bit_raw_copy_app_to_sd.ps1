...
param(
    [string]$SdDrive = "E:",
    [string]$BackupDir = "D:\FPGAhanjia\Hetero_SoC_2026_3\sdcard_backup_20260325_design1bit_raw_copy_app"
)
...
$releaseDir = Join-Path $workspace "sd_boot\ax7020_design1bit_raw_copy_app"
...
Assert-PathType -Path $releaseDir -PathType Container -Label "Design1-bit raw-copy-app release directory"
foreach ($leafPath in @($sourceBoot, $sourceBif, $sourceBootRead, $sourceReadme)) {
    Assert-PathType -Path $leafPath -PathType Leaf -Label "Design1-bit raw-copy-app release artifact"
}

Assert-TextContains -Path $sourceBootRead -RequiredText @("fsbl.elf", "design_1_wrapper.bit", "ax7020_dma_raw_copy_smoke_app.elf") -Label "bootgen_read.txt"
Assert-TextContains -Path $sourceReadme -RequiredText @("design1-bit + raw-copy-app", "RAWCOPY_STAGE INIT") -Label "readme.txt"
...
Write-Host "Design1-bit raw-copy-app release manifest:"
...
if (Test-Path -Path $targetBoot -PathType Leaf) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_before_design1bit_raw_copy_app_$timestamp.bin"
...
Write-Host "Deployed design1-bit raw-copy-app BOOT.BIN to SD:"
Write-Host $targetBoot
Write-Host "SHA256: $targetHash"
Write-Host "Expected board-side behavior:"
Write-Host "  - UART should print at least 'RAWCOPY_STAGE INIT'"
Write-Host "  - If this image prints while dedicated raw-copy images stay silent, the blocker is in the dedicated raw-copy hardware/boot chain."
