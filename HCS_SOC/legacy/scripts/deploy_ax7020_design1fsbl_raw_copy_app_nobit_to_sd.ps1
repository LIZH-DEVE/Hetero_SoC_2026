...
param(
    [string]$SdDrive = "E:",
    [string]$BackupDir = "D:\FPGAhanjia\Hetero_SoC_2026_3\sdcard_backup_20260325_design1fsbl_raw_copy_app_nobit"
)
...
$releaseDir = Join-Path $workspace "sd_boot\ax7020_design1fsbl_raw_copy_app_nobit"
...
Assert-PathType -Path $releaseDir -PathType Container -Label "Design1-FSBL raw-copy-app no-bit release directory"
foreach ($leafPath in @($sourceBoot, $sourceFsbl, $sourceApp, $sourceBootRead, $sourceReadme)) {
    Assert-PathType -Path $leafPath -PathType Leaf -Label "Design1-FSBL raw-copy-app no-bit release artifact"
}

Assert-TextContains -Path $sourceBootRead -RequiredText @("fsbl.elf", "ax7020_dma_raw_copy_smoke_app.elf") -Label "bootgen_read.txt"
Assert-TextContains -Path $sourceReadme -RequiredText @("Raw-Copy-App No-Bit Diagnostic", "RAWCOPY_STAGE INIT") -Label "readme.txt"
...
Write-Host "Design1-FSBL raw-copy-app no-bit release manifest:"
...
if (Test-Path -Path $targetBoot -PathType Leaf) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_before_design1fsbl_raw_copy_app_nobit_$timestamp.bin"
...
Write-Host "Deployed design1-FSBL raw-copy-app no-bit BOOT.BIN to SD:"
...
Write-Host "  - If this boots while hybrid no-bit stays silent, the blocker is in raw-copy FSBL/handoff."
