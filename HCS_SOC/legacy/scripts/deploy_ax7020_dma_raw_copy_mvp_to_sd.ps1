...
param(
    [string]$SdDrive = "E:",
    [string]$BackupDir = "D:\FPGAhanjia\Hetero_SoC_2026_3\sdcard_backup_20260327_dma_raw_copy_mvp"
)
...
$releaseDir = Join-Path $workspace "sd_boot\ax7020_dma_raw_copy_mvp"
$sourceBoot = Join-Path $releaseDir "BOOT.BIN"
$sourceFsbl = Join-Path $releaseDir "fsbl.elf"
$sourceBit = Join-Path $releaseDir "dma_raw_copy_mvp.bit"
$sourceApp = Join-Path $releaseDir "ax7020_dma_raw_copy_mvp_app.elf"
...
Assert-PathType -Path $releaseDir -PathType Container -Label "Raw-copy MVP release directory"
foreach ($leafPath in @($sourceBoot, $sourceFsbl, $sourceBit, $sourceApp, $sourceBootRead, $sourceReadme)) {
    Assert-PathType -Path $leafPath -PathType Leaf -Label "Raw-copy MVP release artifact"
}

Assert-TextContains -Path $sourceBootRead -RequiredText @("fsbl.elf", "dma_raw_copy_mvp.bit", "ax7020_dma_raw_copy_mvp_app.elf") -Label "bootgen_read.txt" | Out-Null
Assert-TextContains -Path $sourceReadme -RequiredText @("DMA raw-copy MVP image", "DMA raw-copy MVP PASS") -Label "readme.txt" | Out-Null
...
$releaseArtifacts = [ordered]@{
    "BOOT.BIN"                     = $sourceBoot
    "fsbl.elf"                     = $sourceFsbl
    "dma_raw_copy_mvp.bit"         = $sourceBit
    "ax7020_dma_raw_copy_mvp_app.elf" = $sourceApp
    "bootgen_read.txt"             = $sourceBootRead
    "readme.txt"                   = $sourceReadme
}

Write-Host "Raw-copy MVP release manifest:"
...
if (Test-Path -Path $targetBoot -PathType Leaf) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_before_dma_raw_copy_mvp_$timestamp.bin"
...
Write-Host "Deployed raw-copy MVP BOOT.BIN to SD:"
...
Write-Host "  4. ax7020_dma_raw_copy_mvp -> expect ring-full rc=-5 and IRQ coalescing count/timeout proof"
