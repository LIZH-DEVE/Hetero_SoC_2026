...
param(
    [string]$SdDrive = "E:",
    [string]$BackupDir = "D:\FPGAhanjia\Hetero_SoC_2026_3\sdcard_backup_20260325_dma_raw_copy_smoke_nobit"
)
...
$releaseDir = Join-Path $workspace "sd_boot\ax7020_dma_raw_copy_smoke_nobit"
...
Assert-PathType -Path $releaseDir -PathType Container -Label "Raw-copy no-bit release directory"
foreach ($leafPath in @($sourceBoot, $sourceFsbl, $sourceApp, $sourceBootRead, $sourceReadme)) {
    Assert-PathType -Path $leafPath -PathType Leaf -Label "Raw-copy no-bit release artifact"
}

Assert-TextContains -Path $sourceBootRead -RequiredText @("fsbl.elf", "ax7020_dma_raw_copy_smoke_app.elf") -Label "bootgen_read.txt" | Out-Null
Assert-TextContains -Path $sourceReadme -RequiredText @("No-Bit Diagnostic", "RAWCOPY_STAGE INIT") -Label "readme.txt" | Out-Null
...
Write-Host "Raw-copy no-bit release manifest:"
...
if (Test-Path -Path $targetBoot -PathType Leaf) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_before_dma_raw_copy_smoke_nobit_$timestamp.bin"
...
Write-Host "Deployed raw-copy no-bit diagnostic BOOT.BIN to SD:"
