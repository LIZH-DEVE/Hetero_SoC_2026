...
param(
    [string]$SdDrive = "E:",
    [string]$BackupDir = "D:\FPGAhanjia\Hetero_SoC_2026_3\sdcard_backup_20260327_dma_stream_smoke_nobit"
)
...
$releaseDir = Join-Path $workspace "sd_boot\ax7020_dma_raw_copy_stream_smoke_nobit"
...
Assert-PathType -Path $releaseDir -PathType Container -Label "Stream-smoke no-bit release directory"
foreach ($leafPath in @($sourceBoot, $sourceFsbl, $sourceApp, $sourceBootRead, $sourceReadme)) {
    Assert-PathType -Path $leafPath -PathType Leaf -Label "Stream-smoke no-bit release artifact"
}

Assert-TextContains -Path $sourceBootRead -RequiredText @("fsbl.elf", "ax7020_dma_raw_copy_stream_smoke_app.elf") -Label "bootgen_read.txt" | Out-Null
Assert-TextContains -Path $sourceReadme -RequiredText @("No-Bit Diagnostic", "DMA stream smoke image") -Label "readme.txt" | Out-Null
...
Write-Host "Stream-smoke no-bit release manifest:"
...
if (Test-Path -Path $targetBoot -PathType Leaf) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_before_dma_raw_copy_stream_smoke_nobit_$timestamp.bin"
...
Write-Host "Deployed stream-smoke no-bit diagnostic BOOT.BIN to SD:"
Write-Host $targetBoot
Write-Host "SHA256: $targetHash"
Write-Host "Expected board-side behavior:"
Write-Host "  - UART should print at least 'DMA stream smoke image'"
Write-Host "  - If no-bit prints but with-bit stays silent, the blocker is in bitstream load/PL stage."
