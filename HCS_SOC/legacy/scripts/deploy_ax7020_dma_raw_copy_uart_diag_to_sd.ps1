...
param(
    [string]$SdDrive = "E:",
    [string]$BackupDir = "D:\FPGAhanjia\Hetero_SoC_2026_3\sdcard_backup_20260325_dma_raw_copy_uart_diag"
)
...
$releaseDir = Join-Path $workspace "sd_boot\ax7020_dma_raw_copy_uart_diag"
$sourceBoot = Join-Path $releaseDir "BOOT.BIN"
$sourceFsbl = Join-Path $releaseDir "fsbl.elf"
$sourceBit = Join-Path $releaseDir "dma_raw_copy_uart_diag.bit"
$sourceApp = Join-Path $releaseDir "ax7020_dma_raw_copy_uart_diag_app.elf"
...
foreach ($path in @($sourceBoot, $sourceFsbl, $sourceBit, $sourceApp, $sourceBootRead, $sourceReadme)) {
    Assert-Leaf -Path $path -Label "UART-only raw-copy release artifact"
}

$bootReadText = Get-Content -Path $sourceBootRead -Raw -Encoding ASCII
foreach ($required in @("fsbl.elf", "dma_raw_copy_uart_diag.bit", "ax7020_dma_raw_copy_uart_diag_app.elf")) {
    if ($bootReadText -notlike "*$required*") {
        throw "bootgen_read.txt missing required partition '$required': $sourceBootRead"
    }
}

$readmeText = Get-Content -Path $sourceReadme -Raw -Encoding ASCII
if ($readmeText -notlike "*UART-only diagnostic*") {
    throw "readme.txt does not describe the UART-only diagnostic image: $sourceReadme"
}
...
if (Test-Path -Path $targetBoot -PathType Leaf) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_before_dma_raw_copy_uart_diag_$timestamp.bin"
...
Write-Host "UART-only raw-copy release manifest:"
...
Write-Host "Deployed UART-only raw-copy diagnostic BOOT.BIN to SD:"
