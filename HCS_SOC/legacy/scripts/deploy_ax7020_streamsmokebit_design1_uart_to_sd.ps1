...
param(
    [string]$SdDrive = "E:",
    [string]$BackupDir = "D:\FPGAhanjia\Hetero_SoC_2026_3\sdcard_backup_20260327_streamsmokebit_design1_uart"
)
...
$releaseDir = Join-Path $workspace "sd_boot\ax7020_streamsmokebit_design1_uart"
...
Assert-PathType -Path $releaseDir -PathType Container -Label "streamsmokebit-design1-uart release directory"
foreach ($leafPath in @($sourceBoot, $sourceFsbl, $sourceBit, $sourceApp, $sourceBootRead, $sourceReadme)) {
    Assert-PathType -Path $leafPath -PathType Leaf -Label "streamsmokebit-design1-uart release artifact"
}

Assert-TextContains -Path $sourceBootRead -RequiredText @("fsbl.elf", "stream_smoke_dma_wrapper.bit", "ax7020_repo_design1_uart_baseline_app.elf") -Label "bootgen_read.txt" | Out-Null
Assert-TextContains -Path $sourceReadme -RequiredText @("design1-UART", "REPO DESIGN1 UART BASELINE") -Label "readme.txt" | Out-Null
...
Write-Host "streamsmokebit-design1-uart release manifest:"
...
if (Test-Path -Path $targetBoot -PathType Leaf) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_before_streamsmokebit_design1_uart_$timestamp.bin"
...
Write-Host "Deployed streamsmokebit-design1-uart BOOT.BIN to SD:"
Write-Host $targetBoot
Write-Host "SHA256: $targetHash"
Write-Host "Expected board-side behavior:"
Write-Host "  - UART should print at least 'REPO DESIGN1 UART BASELINE'"
Write-Host "  - If this image stays silent, the dedicated stream-smoke bitstream itself is breaking board bring-up."
