...
$sourceBoot = Join-Path $workspace "sd_boot\ax7020_udp_gateway_design1_hybrid_official_fsbl_ws2_smoke\$TestCase\BOOT.BIN"
$targetBoot = Join-Path $SdDrive "BOOT.BIN"

if (-not (Test-Path $sourceBoot)) {
    throw "ws2 smoke BOOT.BIN not found for test case '$TestCase': $sourceBoot"
}
...
if (Test-Path $targetBoot) {
    ...
    $backupBoot = Join-Path $BackupDir "BOOT_from_sd_before_ws2_smoke_${TestCase}_$timestamp.bin"
...
Write-Host "Deployed ws2 smoke BOOT.BIN to SD:"
