...
set out_xsa [file join $workspace_root "stream_smoke_dma_wrapper.xsa"]
...
if {[info exists ::env(STREAM_SMOKE_EXPORT_DRY_RUN)] && $::env(STREAM_SMOKE_EXPORT_DRY_RUN) eq "1"} {
    set dry_run 1
}
...
foreach src_info {
    ...
    {"rtl/core/dma/stream_dummy_source.sv" "SystemVerilog"}
    {"rtl/top/dma_raw_copy_subsystem.sv" "SystemVerilog"}
    {"rtl/top/dma_stream_smoke_board_wrapper.v" "Verilog"}
} {
...
foreach stale_path [list [file dirname $raw_bd] [file join $workspace_root "HCS_SOC.gen" "sources_1" "bd" $raw_bd_name]] {
    if {[file exists $stale_path]} {
        if {[catch {file delete -force $stale_path} delete_err]} {
            puts "Warning: failed to delete stale stream-smoke artifact $stale_path: $delete_err"
        }
    }
}
...
if {![file exists $raw_bd]} {
    error "stream-smoke BD clone was not created: $raw_bd"
}
...
connect_bd_net [get_bd_pins dma_stream_smoke_subsystem_0/clk] [get_bd_pins processing_system7_0/FCLK_CLK0]
connect_bd_net [get_bd_pins dma_stream_smoke_subsystem_0/rst_n] [get_bd_pins rst_ps7_0_100M/peripheral_aresetn]
...
connect_bd_intf_net [get_bd_intf_pins dma_stream_smoke_subsystem_0/m_axi_dma_wr] [get_bd_intf_pins axi_mem_intercon/S00_AXI]
connect_bd_intf_net [get_bd_intf_pins dma_stream_smoke_subsystem_0/m_axi_fetcher] [get_bd_intf_pins axi_mem_intercon/S01_AXI]
connect_bd_intf_net [get_bd_intf_pins dma_stream_smoke_subsystem_0/m_axi_s2mm] [get_bd_intf_pins axi_mem_intercon/S02_AXI]

assign_bd_address -offset 0x40000000 -range 4K \
    -target_address_space [get_bd_addr_spaces processing_system7_0/Data] \
    [get_bd_addr_segs dma_stream_smoke_subsystem_0/s_axil_dma/reg0] -force
assign_bd_address -offset 0x40001000 -range 4K \
    -target_address_space [get_bd_addr_spaces processing_system7_0/Data] \
    [get_bd_addr_segs dma_stream_smoke_subsystem_0/s_axil_dummy/reg0] -force
...
foreach addr_space {m_axi_dma_wr m_axi_fetcher m_axi_s2mm} {
    assign_bd_address -offset 0x00000000 -range 512M \
        -target_address_space [get_bd_addr_spaces dma_stream_smoke_subsystem_0/$addr_space] \
...
set module_ref_synth_wrapper [file join $workspace_root "HCS_SOC.gen" "sources_1" "bd" $raw_bd_name "ip" "stream_smoke_dma_dma_stream_smoke_subsystem_0_0" "synth" "stream_smoke_dma_dma_stream_smoke_subsystem_0_0.v"]
if {![file exists $module_ref_synth_wrapper]} {
    error "stream-smoke module-ref synth wrapper was not generated: $module_ref_synth_wrapper"
}
...
update_compile_order -fileset sources_1
set_property top stream_smoke_dma_wrapper [current_fileset]
puts "Wrapper files: $wrapper_files"
puts "Top set to: [get_property top [current_fileset]]"

if {$dry_run} {
    puts "Dry-run complete. Stream-smoke hardware graph and wrapper generated without launching synth/impl."
    close_project
    return
}
...
set bit_file [file join $impl_dir "stream_smoke_dma_wrapper.bit"]
if {![file exists $bit_file]} {
    error "bitstream not found after impl: $bit_file"
}
puts "stream-smoke bitstream: $bit_file"
