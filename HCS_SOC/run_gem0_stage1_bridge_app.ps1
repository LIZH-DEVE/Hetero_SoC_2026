...
$programTcl = Join-Path $workspace "download_gem0_stage1_bridge_app.tcl"
...
function Test-XsdbStepSuccess {
    param(
        [string]$StepName,
        [string]$Text
    )

    if (-not $successMarkers.ContainsKey($StepName)) {
        return $true
    }

    return $Text -match [regex]::Escape($successMarkers[$StepName])
}
...
function Invoke-XsdbScriptOnce {
    ...
    if (Test-HardXsdbFailure -Text $text) {
        throw "xsdb $StepName step reported hard failure on attempt $Attempt"
    }

    if (-not (Test-XsdbStepSuccess -StepName $StepName -Text $text)) {
        if (Test-TransientXsdbFailure -Text $text) {
            throw "xsdb $StepName step ended without success marker after transient failure on attempt $Attempt"
        }
        throw "xsdb $StepName step ended without success marker on attempt $Attempt"
    }
}
