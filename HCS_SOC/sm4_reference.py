#!/usr/bin/env python3
"""Minimal SM4 ECB reference helper for board-side acceptance scripts."""
...
def _t(value: int) -> int:
    return _l(_tau(value))


def _t_prime(value: int) -> int:
    return _l_prime(_tau(value))
...
def _expand_round_keys(key: bytes) -> list[int]:
    ...
    rks: list[int] = []
    for i in range(32):
        nxt = k[i] ^ _t_prime(k[i + 1] ^ k[i + 2] ^ k[i + 3] ^ _CK[i])
        k.append(nxt)
        rks.append(nxt)
    return rks
...
def ecb_encrypt(key: bytes, payload: bytes) -> bytes:
    if len(payload) % 16 != 0:
        raise ValueError("SM4 ECB payload must be a multiple of 16 bytes")
    round_keys = _expand_round_keys(key)
    blocks = (
        _encrypt_block(payload[i:i + 16], round_keys)
