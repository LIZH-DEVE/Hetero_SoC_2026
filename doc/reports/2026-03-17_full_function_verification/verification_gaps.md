...
### G-04 FastPath 缺陷状态更新

- 原始结论“FastPath eligible packet 完全不出数”已经不再成立。
- 最新状态：
  - `rtl/core/fast_path.sv` 已修复首拍在判路前被提前消费的问题
  - `tb/tb_fast_path_sanity.sv` 已在修复后通过 `xvlog/xelab/xsim` 全流程复验
  - `tb/tb_day17_fastpath.sv` 已按当前 RTL 合约重写，并完成 7/7 通过复验
- 仍保留的缺口：
  - bypass 路径当前仍是分类/等待外部接管语义，不是本模块内部自行完成的旁路转发链
  - 板级和主机直连网络场景下的 FastPath 真实链路还没有新增运行证据
- 因此，G-04 应继续保持“部分修复，仍需集成验收”，但 Day17 模块级失败已不再是当前主矛盾。
- 2026-03-17 新增说明：
  - `drop_cnt` 位宽 bug 已修复，不再存在 32-bit 到 1-bit 的截断
  - Day17 TB 已补充 back-pressure 和连续包覆盖
  - 仍待补齐的 FastPath 异常恢复覆盖主要是 “中途复位” 与 “传输中途 valid 撤销”

- 2026-03-17 最新状态：
  - FastPath 的 “联合 back-pressure / BYPASS 恢复 / valid 撤销 / 中途 reset” 覆盖已经补齐
  - 因此这份审查报告中的 FastPath 问题项已全部关闭
  - 当前剩余 ARP 风险已转移到 responder 侧真实集成，而不是 parser 侧 dummy 输出

- 2026-03-17 ARP 更新：
  - `rx_parser` 的 ARP dummy 输出已移除
  - `arp_responder` 单体和 `rx_parser -> arp_responder` 最小链路都已有通过证据
  - 因此 ARP 风险已进一步收缩为“完整 TX / 板级 / 主机抓包链”，不再是 parser-responder 最小闭环本身
## 2026-03-17 ARP 增量收口

- 已关闭:
  - `arp_responder` 的 `o_tx_data` 冗余组合回退路径
  - `rx_parser` 的 UDP malformed 旧值污染风险
  - `tb_rx_parser_arp_sanity.sv` / `tb_arp_chain_sanity.sv` 的悬挂 fork 线程风险
  - `tb_arp_responder_sanity.sv` 连续请求场景中手工赋值与 task 混用的问题
- 新证据:
  - `tb_rx_parser_arp_sanity.sv` 现已覆盖“malformed UDP 后接合法 UDP”的跨包回归
  - `tb_arp_responder_sanity.sv` 和 `tb_arp_chain_sanity.sv` 在修改后继续保持通过
- 因此当前 ARP 相关剩余风险继续收缩为:
  - 完整 TX 构帧
  - 板级真实网络回包
  - 主机抓包链
