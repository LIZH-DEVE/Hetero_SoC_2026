...
module fast_path #(
    ...
    // Hold the first beat until the path decision is complete. Otherwise the
    // stream source can consume beat 0 before FAST_PATH_TX is active.
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (state == FAST_PATH_TX && s_axis_tlast && s_axis_tvalid && s_axis_tready) begin
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Bypass/drop are classification events. This block does not own an
            // alternate output path, so count the decision once when CHECK_PATH
            // resolves to a non-fast-path outcome.
            ...
            if (state == FAST_PATH_TX && s_axis_tlast && s_axis_tvalid && s_axis_tready && checksum_valid) begin
