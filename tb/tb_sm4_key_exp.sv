...
module tb_sm4_key_exp();
    ...
    // Expected round keys from GM/T 0002-2012
    // rk0 = F12186F9, rk1 = 41662B61, rk2 = 5A6AB19A, rk3 = 7BA92077
    // Historical debug values (F09279A1, 0A2F3E83, ...) are obsolete.
    ...
    initial begin
        $display("========================================");
        $display("  SM4 Key Expansion Test");
        $display("========================================");
        ...
        $display("\nKey: %h", SM4_KEY);
        $display("\nExpected round keys (GM/T 0002-2012):");
        $display("  rk0  = F12186F9, rk1  = 41662B61, rk2  = 5A6AB19A, rk3  = 7BA92077");
        
        // Set up key
        user_key_in = SM4_KEY;
        encdec_sel_in = 1'b0;  // Normal order for encryption
        ...
        // The expected values might need byte swapping
        $display("\n[3] Checking byte order:");
        $display("    If we byte-swap each 32-bit word:");
        for (int i = 0; i < 4; i++) begin
            logic [31:0] swapped;
