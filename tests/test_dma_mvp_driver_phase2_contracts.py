...
class TestDmaMvpDriverPhase2Contracts(unittest.TestCase):
    def test_header_declares_phase2_irq_and_ring_helpers(self):
        ...
        for symbol in (
            ...
            "dma_ring_counter_clear",
            "dma_ring_counter_snapshot",
            "dma_ring_read_counter_snapshot",
        ):
    ...
    def test_counter_snapshot_helpers_use_counter_ctrl_and_split_64bit_csrs(self):
        header_text = DRIVER_H.read_text(encoding="ascii")
        impl_text = DRIVER_C.read_text(encoding="ascii")
        contract_text = CONTRACT_H.read_text(encoding="ascii")
        ...
        for token in (
            "typedef struct {",
            "uint64_t rollback_event_count;",
            "uint64_t backend_accept_cycles;",
            "uint64_t drop_pulse_count;",
            "dma_counter_snapshot_t;",
        ...
        for token in (
            "DMA_CSR_COUNTER_CTRL",
            "DMA_CSR_ROLLBACK_EVENT_COUNT_LO",
            "DMA_CSR_BACKEND_ACCEPT_CYCLES_HI",
            "DMA_COUNTER_CTRL_CLEAR_ALL",
            "DMA_COUNTER_CTRL_SNAPSHOT",
            "dma_ring_read_counter64(",
            "void dma_ring_counter_clear(const dma_ring_ctx_t *ctx)",
            "void dma_ring_counter_snapshot(const dma_ring_ctx_t *ctx)",
            "int dma_ring_read_counter_snapshot(const dma_ring_ctx_t *ctx, dma_counter_snapshot_t *out)",
        ):
            self.assertIn(token, impl_text)

        for token in (
            "DMA_CSR_COUNTER_CTRL",
            "DMA_CSR_ROLLBACK_EVENT_COUNT_LO",
            "DMA_CSR_DROP_PULSE_COUNT_HI",
            "DMA_COUNTER_CTRL_CLEAR_ALL",
            "DMA_COUNTER_CTRL_SNAPSHOT",
        ):
