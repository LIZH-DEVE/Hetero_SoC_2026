...
class TestUdpGatewayShadowRuntimeContracts(unittest.TestCase):
    ...
    def test_crypto_dma_subsystem_exports_counter_snapshot_path_over_existing_csr_chain(self):
        text = self.crypto_dma_subsystem

        for token in (
            "csr_counter_clear",
            "csr_counter_snapshot",
            "pbm_rollback_active",
            "pbm_high_water",
            "pbm_drop_pulse",
            "pbm_rollback_pulse",
            "counter_backend_total_cycles_q",
            "counter_backend_accept_cycles_q",
            "counter_backend_starvation_cycles_q",
            ".o_counter_clear(csr_counter_clear)",
            ".o_counter_snapshot(csr_counter_snapshot)",
            ".i_counter_rollback_event_count(",
            ".i_counter_backend_accept_cycles(",
            ".i_counter_drop_pulse_count(",
            ".o_high_water(pbm_high_water)",
            ".o_drop_pulse(pbm_drop_pulse)",
            ".o_rollback_pulse(pbm_rollback_pulse)",
        ):
            self.assertIn(token, text)
